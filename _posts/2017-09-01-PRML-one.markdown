...
- 1. **机器学习要解决的根本问题**
    -  机器学习的必要不充分条件:在训练集的数据和测试集的数据需来源同一分布或者两个相近的分布的前提下,且要做到以下两点：
        ...
        - (2) 使得测试误差要逼近训练误差的值$E_{test}\appro E_{train}$(测试过程，保证了模型的泛化能力)，这样就保证了测试误差也很小。  
...
**如何保证$E_{test}\appro E_{train}$？**这要涉及到VC-dimension的理论，这里直接抛出结论，具体推导亦在[机器学习基石](https://www.coursera.org/learn/ntumlone-mathematicalfoundations)课程中有：
...
2. 当数据样本$N$一定时，若**d_{vc}(H)**很小，说明学习到的模型复杂度低，这会使得$E_{train}$很高，即使条件(2)可以满足:$E_{test}\appro E_{train}$，但$E_{test}$很大，学习效果很差。这属于欠拟合现象，解决方法是增大vc维度。
...
$$\begin{align}
E_{D}[E_{test}[g^{D}]] = E_{D}[E_{test}[\lVert g^{D}-f(x) \rVert^2]]
\end{align}
$$
