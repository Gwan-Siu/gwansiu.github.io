...
$$
\begin{align}
\displaystyle{p(X,Y)&=\prod_{t=1}^{T}p(x_{t}\vert y_{t})p(y_{t}\vert y_{t-1})} \\
...
$$
\begin{equaiton}
\xi_{ij}(t)=\frac{p(y_{t}=i,y_{t+1}=j, X\vert\lambda)}{p(X\vert\lamda)}=\frac{\alpha_{i}(t)a_{ij}b_{j}(x_{t+1})\beta_{j}(t+1)}{\sum_{i=1}^{K}\sum_{j}^{K}\alpha_{i}(t)a_{ij}b_{j}(x_{t+1})\beta_{j}(t+1)}
\end{equation}
$$
...
$$
\begin{equaiton}
\xi_{ij}(t)=\frac{p(y_{t}=i,y_{t+1}=j, X\vert\lambda)}{p(X\vert\lamda)}=\frac{y_{i}(t)a_{ij}b_{j}(x_{t+1})\beta_{j}(t+1)}{\beta_{i}(t)}
\end{equation}
$$
